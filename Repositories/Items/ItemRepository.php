...
class ItemRepository implements ItemRepositoryInterface
{
    ...
    public function getById(int $id): ItemDTO
    {
        ...
        return new ItemDTO($item['title'], $item['company'], $item['salary'], $item['description'], $item['posted_on']);
    }
    ...
    public function editItem(ItemDTO $itemDTO, int $id)
    {
        ...
        $this->db->query("UPDATE job_offers SET title=?, company=?, salary=?, description=? WHERE id = ?")
            ->execute($params);
    }
